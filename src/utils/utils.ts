...
export const addBankSchema = Joi.object().keys({
    ...
    accountNumber: Joi.string().length(10).regex(/^[0-9]*$/).required()
      .messages({
  ...
  export const addTransactionSchema = Joi.object().keys({
    ...
    destinationPhoneNumber: Joi.string().length(11).pattern(/^[0-9]+$/).required()
  });
  ...
  export const generateToken = (user: string, expiryDate: string): string => {
